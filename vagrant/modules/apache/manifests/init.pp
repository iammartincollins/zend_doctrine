class apache {
  ...
  file {"/etc/apache2/sites-enabled":
    ...
    purge => true,
    force => true,
    before => File["/etc/apache2/sites-enabled/vagrant_zend"],
    require => Package["apache2"],
  }

  # create apache config from main vagrant manifests
  file { "/etc/apache2/sites-available/vagrant_zend":
    ensure => present,
    source => "/var/www/vagrant/manifests/vagrant_zend",
    require => Package["apache2"],
  }

  # symlink apache site to the site-enabled directory
  file { "/etc/apache2/sites-enabled/vagrant_zend":
    ensure => link,
    target => "/etc/apache2/sites-available/vagrant_zend",
    require => File["/etc/apache2/sites-available/vagrant_zend"],
